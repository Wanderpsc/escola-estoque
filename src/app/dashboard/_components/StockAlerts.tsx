...
export default function StockAlerts({ products }: Props) {
  ...
  return (
    <div className="bg-white rounded-xl border border-slate-200 shadow-sm h-full">
      <div className="flex items-center gap-2 px-5 py-4 border-b border-slate-100">
        <AlertTriangle className={cn("w-4 h-4", alerts.length > 0 ? "text-red-500" : "text-green-500")} />
        <h3 className="text-sm font-semibold text-slate-700">Alertas de Estoque</h3>
        {alerts.length > 0 && (
      ...
      <div className="overflow-y-auto max-h-80">
        {alerts.length === 0 ? (
          <div className="flex flex-col items-center justify-center py-10 text-slate-400">
            ...
            <p className="text-sm">Todos os estoques OK!</p>
          </div>
        ) : (
