...
const DATA_MODELS: ModelDef[] = [
  ...
  },
  {
    ...
    fields: [
      ...
      { name: "isExtra", type: "Boolean — saída sem NF → gera BudgetMovement DEBIT automático" },
    ],
  ...
  },
  {
    ...
    fields: [
      ...
      { name: "reference", type: "String? (PURCHASE-{id} | NF-EXTRA-{id} | DELIVERY-{id})" },
...
const API_ROUTES: ApiRoute[] = [
  ...
  { method: "POST", path: "/api/stock/exits", roles: "Escola + checkLicense", description: "Registra saída; se isExtra=true cria BudgetMovement DEBIT automático" },
...
const BUSINESS_FLOWS: Flow[] = [
  ...
  },
  {
    ...
    steps: [
      { label: "Usuário seleciona Programa + motivo + data + itens", type: "action" },
      { label: "Marca 'isExtra' se a saída deve debitar orçamento", type: "branch" },
      { label: "POST /api/stock/exits → valida → checkLicense", type: "db" },
      { label: "Cria StockExit + ExitItems", type: "db" },
      { label: "Se isExtra=true → cria BudgetMovement DEBIT EXTRA automático", type: "branch" },
      { label: "Saldo de estoque diminui; orçamento debitado se isExtra", type: "result" },
    ],
  },
  {
  ...
  },
  {
    ...
    steps: [
      { label: "Créditos: BudgetMovements type=CREDIT (repasses, ajustes manuais)", type: "action" },
      { label: "Débitos NF: StockEntry.totalValue (apenas itens NF oficiais)", type: "action" },
      { label: "Débitos Extras: BudgetMovements type=DEBIT (compras informais, extras NF, saídas extra, entregas)", type: "action" },
      { label: "Saldo = Σ(CREDIT) - Σ(DEBIT NF) - Σ(DEBIT BudgetMovements)", type: "result" },
      { label: "Exibido por programa e consolidado no Financeiro", type: "result" },
    ],
  },
  {
...
const MIGRATIONS = [
  ...
  { version: "20260804020000_add_stock_exit_is_extra", desc: "StockExit.isExtra — saída gera débito financeiro automático" },
