      ...
      <Modal open={modal} onClose={() => { setModal(false); setHeader(EMPTY_HEADER); setItems([{ ...EMPTY_ITEM }]); }} title="Registrar Compra Informal" size="lg">
        <div className="space-y-4">
          ...
          <div>
            ...
            <div className="grid grid-cols-[2fr_1fr_2fr_1fr_auto] gap-2 text-xs font-semibold text-slate-400 uppercase px-1 mb-1">
            ...
            <div className="space-y-2">
              {items.map((row, i) => {
                ...
                return (
                  <div key={i} className="grid grid-cols-[2fr_1fr_2fr_1fr_auto] gap-2 items-center bg-white border border-slate-200 rounded-lg px-3 py-2">
                    ...
                    <input type="number" step="0.001" min="0" value={row.quantity} onChange={(e) => setItemField(i, "quantity", e.target.value)} className="border border-slate-300 rounded-lg px-2 py-1.5 text-sm focus:outline-none focus:ring-1 focus:ring-amber-400 w-full" />
                    <input type="number" step="0.01" min="0" placeholder="0,00" value={row.unitPrice} onChange={(e) => setItemField(i, "unitPrice", e.target.value)} className="border border-slate-300 rounded-lg px-2 py-1.5 text-sm focus:outline-none focus:ring-1 focus:ring-amber-400 w-full" />
